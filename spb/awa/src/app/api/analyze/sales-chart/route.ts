...
import { salesChartData } from '@/data/mock-data';

export async function GET() {
  try {
    // Here you would typically fetch data from your actual API
    // For example:
    // const response = await fetch('https://your-api-url/analyze/sales-chart');
    ...
    return NextResponse.json(salesChartData);
    ...
    return new NextResponse(JSON.stringify({ error: 'Failed to fetch sales chart data' }), {
      status: 500,
      headers: {
